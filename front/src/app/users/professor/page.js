...
export default function ProfessoresPage() {
  ...
  const carregarProfessores = async () => {
    try {
      ...
      const apenasProfessores = (response.data || []).filter(u => u.profile === 'PROFESSOR');
  ...
  const handleDelete = async (user) => {
    if (window.confirm(`Excluir o professor "${user.username}"?`)) {
      try {
  ...
  return (
    <PageLayout
      ...
      subtitle="Corpo docente cadastrado"
      topRightAction={<Button href="/cadastro?perfil=PROFESSOR">Novo Professor</Button>}
      bottomLeftAction={<Button href="/menu">Voltar</Button>}
    >
      {error && <p className="error-message">{error}</p>}

      {isLoading ? <div>Carregando...</div> : (
        <Table data={professores} columns={columns} onEdit={handleEditClick} onDelete={handleDelete} />
      )}

      {editingUser && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h2 className="modal-header">Editar Professor</h2>
