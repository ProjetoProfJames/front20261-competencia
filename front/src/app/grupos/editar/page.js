...
function FormularioEdicaoGrupo() {
  ...
  useEffect(() => {
    async function carregarDados() {
      ...
      if (idGrupo) {
        try {
          const grupo = await API.get(`/projetos/${idGrupo}`);
          setCampoNome(grupo.nome || "");
          setCampoDescricao(grupo.descricao || "");
          setTurmaSelecionada(grupo.turma?.id?.toString() || "");
          setSemestreSelecionado(grupo.semestre?.id?.toString() || "");
          setProfessorSelecionado(grupo.professorOrientador?.id?.toString() || "");
          setLocalSelecionado(grupo.local?.id?.toString() || "");
          setAlunosSelecionados(grupo.integrantes?.map((i) => i.id) || []);
          if (grupo.horarioInicio) {
            const dt = new Date(grupo.horarioInicio);
            setCampoData(dt.toISOString().slice(0, 10));
            setHoraAbertura(dt.toISOString().slice(11, 16));
          }
          if (grupo.horarioFim) {
            const dt = new Date(grupo.horarioFim);
            setHoraEncerramento(dt.toISOString().slice(11, 16));
          }
        } catch (erro) {
          console.error("Falha ao carregar dados do grupo:", erro);
        }
      }
    }
  ...
  const processarFormulario = async (evento) => {
    ...
    try {
      await API.put(`/projetos/${idGrupo}`, payload);
  ...
  return (
    <div style={{ padding: "30px", maxWidth: "600px", margin: "0 auto", fontFamily: "sans-serif" }}>
      <h2>Modificar Informações do Grupo</h2>
...
export default function TelaEditarGrupo() {
  return (
    <Suspense fallback={<div style={{ padding: "30px" }}>Carregando formulário...</div>}>
